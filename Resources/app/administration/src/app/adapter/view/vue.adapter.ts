...
import setupSnapAdminDevtools from 'src/app/adapter/view/sw-vue-devtools';
import type ApplicationBootstrapper from 'src/core/application';
import type { ComponentConfig } from 'src/core/factory/async-component.factory';
import type { ComponentPublicInstance } from '@vue/runtime-core';

const { Component, State, Mixin } = SnapAdmin;
...
export default class VueAdapter extends ViewAdapter {
    ...
    constructor(Application: ApplicationBootstrapper) {
        ...
        this.vueComponents = {};
        this.app = createApp({ name: 'SnapAdminAdministration', template: '<sw-admin />' });
    }
    ...
    initVue(renderElement: string, router: Router, providers: { [key: string]: unknown }): App<Element> {
         ...
         *
         * So we should convert from provide/inject to SnapAdmin.Service
         */
        ...
        if (process.env.NODE_ENV === 'development') {
            setupSnapAdminDevtools(this.root);
        }
    ...
    initLocales(store: Store<VuexRootState>) {
        ...
        const messages = {};
        const fallbackLocale = SnapAdmin.Context.app.fallbackLocale as FallbackLocale;
    ...
    setLocaleFromUser(store: Store<VuexRootState>) {
        ...
        if (currentUser) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
            SnapAdmin.Service('localeHelper').setLocaleWithId(userLocaleId);
        }
    }
